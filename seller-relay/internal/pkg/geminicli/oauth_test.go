...
func TestBuildAuthorizationURL_RequiresConfiguredBuiltinSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("BuildAuthorizationURL() 未配置内置 secret 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Fatalf("错误信息应提示设置环境变量，实际: %v", err)
	}
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("不设置环境变量时应返回内置 secret 缺失错误")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Fatalf("错误信息应提示设置环境变量，实际: %v", err)
	}
	if strings.TrimSpace(cfg.ClientSecret) != "" {
		t.Errorf("ClientSecret 应为空，实际: %q", cfg.ClientSecret)
	}
}
