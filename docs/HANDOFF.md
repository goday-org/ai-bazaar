...
2. 让 reviewer 把该账号加为 repo collaborator（推荐 **Triage**；User-owned repo 只能给 **push**，见下方说明）
3. 该账号是所有 PR 的 author / commit author
4. CODEOWNERS 仍指向 reviewer，由 reviewer 来 approve

> Triage role 只在 **Organization-owned** repo 上可用。若仓库 owner 是 **User account**（如 `goday-org` 当前形态），API 只接受 `pull` / `push` / `admin`——传 `triage` 会返回 `422 Unprocessable Entity`。这种情况下退回到 `push`：Codex 仍然不能 push main（受 branch ruleset 拦截，需 PR + approval + 通过 CI），但能 push 到非保护分支并开 PR，工作流同样成立。

**reviewer 应做的**（一次性）：

1. 接到 Codex 的账号名后，运行（**先试 triage，失败再退回 push**）：
   ```bash
   # 推荐：org-owned repo
   gh api -X PUT repos/goday-org/ai-bazaar/collaborators/<codex-username> \
     -f permission=triage

   # 若上一条返回 422（owner 是 User account），改用：
   gh api -X PUT repos/goday-org/ai-bazaar/collaborators/<codex-username> \
     -f permission=push
   ```
