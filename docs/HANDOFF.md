...
- **任何"我觉得这样更好"的偏离**：必须在 commit message 里写明 `DEVIATION: 原计划 X，我改成 Y，原因 Z`，让 reviewer 能快速看到。
...
读完所有文档后，**不要直接写代码**。先做这件事：

1. 在仓库根目录建 `.gitignore`、`README.md`（一句话项目简介 + 指向 docs/）、`LICENSE`（AGPL-3.0）
2. 把当前 docs/ 目录提交一个初始 commit：`chore: import handoff docs from reviewer`
3. 回 reviewer："已完成初始化，准备开始 W1"
4. 等 APPROVED 后再开始 W1
